...
def get_post_url(post):
    ...
    date = post.meta['date']
    name = path[9:]
    return '/historias/{}/{}/{}/{}/'.format(date.year, date.month, date.day, name)
...
@app.route('/historias/<int:year>/<int:month>/<int:day>/<path:path>/')
...
@app.route('/historias/tags/<string:tag>/')
def blog_posts_by_tag(tag):
    ''' Blog posts by tag '''
    # Render the page
    page = pages.get_or_404(add_l10n_prefix('historia'))
...
@app.route('/historias/<string:category>/<int:year>/<int:month>/<int:day>/<path:path>/')
...
@app.route('/historias/<string:category>/')
