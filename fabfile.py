...
FRONTENDDIR = abspath(join(abspath(BASEDIR), 'static'))
...
#     project_dir = '/home/gastosabertos/gastosabertos_website'
#     with cd(project_dir):
#         run("git pull")
#         with prefix("source /home/gastosabertos/.virtualenvs/ga-ws/bin/activate"):
...
def get_gulp():
    return check_for('gulp', 'You need to install Gulp to be able to compile front-end JS, stylesheets and run front-end tests', 'sudo npm install -g gulp-cli')
...
def build():
    with lcd(FRONTENDDIR):
        cmd = '%(grunt)s build' % {'grunt': get_gulp()}
        local(cmd)

@task
def develop():
    with lcd(FRONTENDDIR):
        cmd = '%(grunt)s develop' % {'grunt': get_gulp()}
        local(cmd)
